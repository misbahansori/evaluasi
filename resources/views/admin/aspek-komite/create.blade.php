        ...
        <div class="col-md-8">
            <div class="card">
                <div class="card-header bg-white">
                    <h4>Tambah Data Aspek Komite</h4>
                </div>

                <div class="card-body">
                    <form action="{{ route('aspek-komite.store') }}" class="form" method="POST">
                        @csrf
                            ...
                            <div class="col-md-6">
                                <select name="komite_id" id="komite_id" class="form-control{{ $errors->has('komite_id') ? ' is-invalid' : '' }}">
                                    @foreach ($listKomite as $komite)
                                        <option {{ old('komite_id') == $komite->id ? 'selected' : '' }} value="{{ $komite->id }}">{{ $komite->nama }}</option>
                                    @endforeach
                                </select>
                                @error('komite_id')
                        ...
                        <div class="form-group row">
                            ...
                            <div class="col-md-6">
                                <input id="kategori" type="text" class="form-control{{ $errors->has('kategori') ? ' is-invalid' : '' }}" name="kategori" value="{{ old('kategori') }}" required placeholder="Ketikan kategori">
